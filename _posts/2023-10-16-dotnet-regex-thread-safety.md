...
public static string ParseItemIdentity(HttpContext httpContext, string template)
{
    ...
    var allMatches = TemplateToRegexMatchResultMapping.GetOrAdd(template, ParameterPattern.Matches(template));
    
    // Then replace the parameters with the values in httpContext
    StringBuilder stringBuilder = new StringBuilder(GetStringBeforeFirstParameter(template, allMatches[0]));
